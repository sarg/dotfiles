...
(pin "emacs-org-contacts"
     #:commit "41f10b9ab07f267613d77e98ef7a33b1772292b0"
     #:hash "1aqvq1wca7csi7mrgy1gvy1lgq6ybi9ap030zwk697hvz8i0d59g")
(pin "emacs-marginalia"
     #:commit "d38041df2c2d175040bbdb1df6e4cc75a75ca4f8"
     #:hash "180v5y24rjrbh4dk66x3xhx3vb52yj520d61bkxrx790555a0y8g")

(let* ((emms (pkg "emacs-emms"))
       (emms-meta (cadr (assoc "emacs-emms-print-metadata" (package-inputs emms))))
       (meta-fixed
        (package
          (inherit emms-meta)
          (inputs (modify-inputs (package-inputs emms-meta)
                    (append (pkg "zlib")))))))
  (cons emms
        (package
          (inherit emms)
          (inputs (modify-inputs (package-inputs emms)
                    (replace (package-name emms-meta) meta-fixed))))))
#+end_src
#+begin_src scheme :noweb-ref guix-pkg
home-emacs
...
(doom!
 ...
 :lang        (clojure +tree-sitter) data emacs-lisp (javascript +tree-sitter +lsp) markdown
              (org +contacts +dragndrop +pandoc +hugo +roam2 +pretty) beancount (go +lsp +tree-sitter) (rust +lsp +tree-sitter)
...
#+END_src
#+begin_src elisp
;; https://codeberg.org/divyaranjan/emacs-reader/issues/166
;; (after! mailcap
;;   (setq! mailcap-user-mime-data '((reader-mode "application/pdf"))))

;; (after! eww
;;   (setq! eww-retrieve-command (list (guix/pkg 'rdrview "/bin/rdrview" "-H" "-E" "utf-8"))))
#+end_src
** yeetube
#+begin_src scheme :noweb-ref guix-pkg
"emacs-yeetube"
