...
(doom!
 ...
 :ui          nav-flash (modeline +light) ophints (popup +defaults) ligatures
 ...
 :term        eshell vterm
 :tools       editorconfig (eval +overlay) (lookup +docsets) magit pdf (pass +auth) make tree-sitter lsp
 :checkers    syntax
 ...
 :app         calendar telega (rss +org) torrent irc exwm emms
 :email       (mu4e +gmail)
...
(use-package! ednc
  :hook ((exwm-init . ednc-mode)
         (ednc-view-mode . dont-kill-buffer-mode)
         (ednc-view-mode . doom-mark-buffer-as-real-h))

  :config
  (set-popup-rule!
      '(major-mode . ednc-view-mode)
      :ttl nil
      :quit t
      :select t
      :height 5)

  (map! :mode ednc-view-mode
        :n
        "TAB" #'ednc-toggle-expanded-view
        "RET" #'ednc-invoke-action)

  (defun pop-to-ednc (old new)
    (when new (pop-to-buffer ednc-log-name)))

  (add-hook 'ednc-notification-presentation-functions #'pop-to-ednc))
#+end_src
* completion
#+begin_src elisp
...
(package! mini-echo :built-in t)
(package! noether :recipe (:local-repo "~/devel/ext/noether"))
#+end_src
#+begin_src elisp
(use-package! noether
  :config
  ...
  (defun noether--exwm-workspace-format (_ v _ _)
    "Format workspace name"
    (nth v exwm-workspace-names))

  (noether-defunit noether-exwm-workspace-unit
    "Show the exwm workspace."
    :len 4
    :var 'exwm-workspace-current-index
    :fn #'noether--exwm-workspace-format)

  (noether-defunit noether-battery
    "Show battery."
    :len 4
    :var 'battery-mode-line-string
    :fn (lambda (_ v _ _) (format "%4s" v)))

  (noether-defunit noether-kbd-layout-unit
    "Show alternative layout when enabled."
    :len 2
    :var '(exwm-input-current-group current-input-method)
    :fn (lambda (s v _ _)
          (let ((layout (pcase major-mode
                          ('exwm-mode
                           (when (> v 0) (nth v exwm/input-layout-names)))
                          (_ v))))

            (propertize
             (upcase (substring (or layout "  ") 0 2))
             'face 'bold))))

  (defun noether-init ()
    (noether-defview main-bar
      "Main bar"
      ...
      :frame
      (list
       :position (cons (- (frame-outer-width) 80)
                       (- (frame-outer-height) 16))
       :z-group 'above)

      :units
      (list
       (noether-battery)
       (noether-kbd-layout-unit)
       (noether-exwm-workspace-unit)
       (time-unit :label nil)))
    (setq noether-views (list main-bar))
    (noether-global-mode 1))

  (setq battery-mode-line-format "%b%>2p%")
  (display-battery-mode 1)
  (add-hook! exwm-init #'noether-init))
#+end_src
#+begin_src elisp
(defun xkb-layout-segment ()
  (pcase major-mode
    ('exwm-mode
     (when-let ((group (exwm-input--current-group))
                ((> group 0)))
       (nth group exwm/input-layout-names)))
    (_ evil-input-method)))
...
(use-package! mini-echo
  ...
  (mini-echo-persistent-function (-const nil))
  (mini-echo-temporary-rule '(:both ("xkb-layout")))
  (mini-echo-persistent-rule '(:both ("time" "battery" "exwm-workspace")))

  :config
  (mini-echo-define-segment "exwm-workspace"
    "Indicator that shows current EXWM workspace name."
    :fetch
    (mini-echo-segment--print (nth exwm-workspace-current-index exwm-workspace-names)))

  (mini-echo-define-segment "xkb-layout"
    "Indicator that shows current xkb layout if it is active."
    :fetch
    (when-let ((layout (xkb-layout-segment)))
      (mini-echo-segment--print (upcase (substring layout 0 2)) 'bold)))
...
(after! app-launcher
  ...
  (advice-add 'app-launcher-list-desktop-files :filter-return
              (lambda (apps)
                (let ((keep '("lutris" "libreoffice-startcenter" "flatseal" "spotify" "calibre-gui"))
                      (del '("flatpak" "emacs" "qview" "scrcpy" "qutebrowser" "darkman"
...
flatpak install --or-update --noninteractive \
...
(package! org-modern-indent
    ...
    :pin "fcd4368476a9c4eadfac4d6f51159d90a15de15a")
(package! corg
  :recipe (:host github :repo "isamert/corg.el")
  :pin "16d2b80fb2ff4daf0ab9bf89e5c9f649431337fc")
(package! org-modern :pin "0e0756f319c637abf424cef96ee35617a7d8c320")
...
(package! vc-fossil :recipe (:local-repo "~/devel/ext/emacs-fossil"))
#+end_src
...
*** player
#+begin_src elisp :tangle packages.el
(package! emms-player-spotify :pin "e553a2157fb2ebf4e0b00594067eea4c680a1940")
#+end_src
#+begin_src elisp
...
(use-package! counsel-spotify
  ...
  (defun counsel-spotify-verify-credentials () (ignore))
...
(use-package! lexic
  ...
  (defun lexic-open-bword-link (link &rest _)
    (lexic-search (substring link 8 (length link)) nil nil 'interactive nil))
...
(map! :leader "om" (lambda () (interactive) (require 'mu4e-transient) (mu4e-transient-menu)))
...
(after! mu4e
  ...
  (setq mu4e-filter-inbox "flag:unread AND NOT flag:trashed AND maildir:/gmail/all"
        ...
        mu4e-refile-folde "/gmail/all"
        sendmail-program "msmtp"
        message-sendmail-f-is-evil t
        message-sendmail-extra-arguments '("--read-envelope-from")
        send-mail-function (lambda () (error "Don't call me"))
...
#+END_src
#+begin_src sh :tangle ~/.local/share/qutebrowser/userscripts/emacsclient-wrapper :shebang #!/bin/sh
emacsclient -e \
...
** gptel
#+begin_src elisp :tangle packages.el
(package! gptel :pin "dfd88798c1ca26b28c1eb1987cb9967a4fc4d712")
#+end_src
