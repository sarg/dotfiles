...
#+end_src
** COMMENT sops
I use only the sops package which is in main guix channel now.
#+begin_src scheme :noweb-ref guix-channel
(channel
 (name 'sops-guix)
...
(operating-system
  (kernel linux)
  ; https://bbs.archlinux.org/viewtopic.php?id=314578
  (kernel-arguments '("quiet" "loglevel=1" "amdgpu.dcdebugmask=0x40000"))
    ...
    (with-imported-modules '((guix build utils))
      #~(begin
          ...
          (make-desktop-entry-file
           (string-append #$output "/share/wayland-sessions/ewm.desktop")
           #:name "EWM"
           #:type "Application"
           #:exec "emacs --fg-daemon -f ewm-start-module"
           #:comment
           '((#f "Emacs Wayland Manager")))))))
  (license #f) (home-page #f) (synopsis #f) (description #f))
#+end_src
...
(service greetd-service-type
        (greetd-configuration
         (greeter-supplementary-groups '("input"))
         (terminals
          (list (greetd-terminal-configuration
                ...
                (greetd-terminal-configuration
                 ...
                 (default-session-command
                   (file-append (pkg "tuigreet") "/bin/tuigreet")))
                (greetd-terminal-configuration
                 (terminal-vt "3"))))))
#+end_src
** base
...
(simple-service
 ...
 (guix-extension
   ...
   (authorized-keys (list <<cfg-nonguix-pub()>>))))
#+end_src
** networking
#+begin_src scheme :noweb-ref system-use
(gnu services ssh)
...
#+end_src
#+begin_src elisp
(cl-defun iwd-manager--format-mode-line-nerd-icons (&key state ssid scanning &allow-other-keys)
  (concat
   " "
   (cond
    (scanning #("󱛄" 0 1 (display (raise 0.05))))
    ((eq state 'connected) #("󰖩" 0 1 (display (raise 0.05))))
    (t #("󰤮" 0 1 (display (raise 0.05)))))
   (if (and ssid (not (string= ssid "SARG"))) (concat " " ssid " ") " ")))

(use-package iwd-manager
  :defer-incrementally t
  :custom
  (iwd-manager-mode-line-formatter #'iwd-manager--format-mode-line-nerd-icons)
  :config
  (iwd-manager-register-agent)
  (when (modulep! :editor evil +everywhere)
    (evil-collection-inhibit-insert-state 'iwd-manager-mode-map)
    (evil-set-initial-state 'iwd-manager-mode-map 'normal)
    (evil-collection-define-key 'normal 'iwd-manager-mode-map
      "s" 'iwd-manager-scan
      "r" 'revert-buffer
      "p" 'iwd-manager-toggle-power
      "c" 'iwd-manager-connect
      "d" 'iwd-manager-disconnect
      "D" 'iwd-manager-delete-network
      (kbd "RET") 'iwd-manager-connect)))
#+end_src
...
#+end_src
#+begin_src elisp
(defun sarg/vpn-start (country)
  "Start VPN in COUNTRY."
  (interactive (list (completing-read "Country: " '("de" "ru") nil t "de")))
  (message "Starting VPN")
  (sarg/telega-msg-send (telega-chat-by-username "DiogenisBot")
                        (concat "/vpn " country))
  (promise-chain
      (sarg/telega-msg-expect
       (telega-match-gen-predicate 'msg
         '(and (contains "VPN started")
               (chat (bot-user (username "DiogenisBot"))))))
    (then (lambda (msg)
            (eshell-command "sudo herd start wireguard-wg0")

            (when (= 1 (call-process "ip" nil nil nil "a" "show" "dev" "wg0"))
              (notifications-notify :title "Hey" :body "wireguard not running"))
            (notifications-notify :title "Hey" :body (telega-msg-content-text msg))))))


(defun sarg/vpn-stop ()
  "Stop VPN."
  (interactive)
  (unless (= 1 (call-process "pgrep" nil nil nil "aria2c"))
    (user-error "Aria2 is still running"))
  (eshell-command "sudo herd stop wireguard-wg0")
  (sarg/telega-msg-send (telega-chat-by-username "DiogenisBot") "/vpn stop")
  (promise-chain
      (sarg/telega-msg-expect
       (telega-match-gen-predicate 'msg
         '(and (or (contains "Stopped.+VPN")
...
"bluez" "tlp" "libratbag"
#+end_src
#+name: kindle-udev-rule
#+begin_src cfg :wrap "src scheme"
SUBSYSTEM=="net", ACTION=="add", ENV{ID_VENDOR}=="Linux_3.0.35-lab126_with_fsl-usb2-udc", \
  RUN+="🐜(pkg "iproute2")🐜/sbin/ip link set %k up", \
  RUN+="🐜(pkg "iproute2")🐜/sbin/ip add add 192.168.15.243/24 dev %k"
#+end_src

#+RESULTS[0417f331ea1ec0c2a5b1619e91042ae35f3d3e90]: kindle-udev-rule
#+begin_src scheme
(mixed-text-file
  "cfg-file"
"SUBSYSTEM==\"net\", ACTION==\"add\", ENV{ID_VENDOR}==\"Linux_3.0.35-lab126_with_fsl-usb2-udc\", \\\n"
...
(udev-rules-service 'kindle (file->udev-rule "98-kindle-network.rules" <<kindle-udev-rule()>>))
(udev-rules-service 'uaccess (udev-rule "50-uaccess.rules" "ACTION==\"add\", SUBSYSTEM==\"misc\", KERNEL==\"udmabuf\", TAG+=\"uaccess\""))
...
#+end_src
#+begin_src scheme :noweb-ref guix-pkg
"podman-compose" "podman-healthcheckd"
#+end_src
#+begin_src scheme :noweb-ref guix-srv
(simple-service
 'podman-healthcheck
 home-shepherd-service-type
 (let ((command #~(list #$(file-append (pkg "podman-healthcheckd") "/bin/podman-healthcheckd")))
       (log-file #~(string-append %user-log-dir "/podman-healthcheckd.log")))
...
(define home-emacs (pkg "emacs-next-pgtk"))

(define (build-with-home-emacs p)
  (package/inherit p
    (arguments
     (substitute-keyword-arguments (package-arguments p)
       ((#:emacs emacs #f) home-emacs)))))

(define* (local-pkg p #:key dir)
  (let ((base (pkg p)))
    (cons base
          (package
            (inherit base)
            (version "local")
            (source (local-file dir #:recursive? #t #:select? (git-predicate dir)))))))

(define* (pin p #:key commit hash uri)
  (let ((base (pkg p)))
    (cons base
          (package
            (inherit base)
            (version (string-append "pin." (if (> (string-length commit) 7)
            ...
            (source
             (origin
               ...
               (uri (git-reference
                      (url (or uri
                               (git-reference-url
                                (origin-uri (package-source base)))))
                      (commit commit)))
               (file-name (git-file-name (package-name base) version))
               (sha256 (base32 hash))))))))

(define pinned (package-input-rewriting (list <<guix-pin>>)))
(define pinned+output
  (match-lambda
    ((pkg output) (list (pinned pkg) output))
    (pkg (pinned pkg))))
(define all-packages (map (compose pinned+output pkg) (flatten (list <<guix-pkg>>))))
...
(".q3a" ,(symlink-to "/storage/data/quake3"))
(".local/share/ddnet" ,(symlink-to "/storage/data/ddnet"))
#+end_src
** media
#+begin_src scheme :noweb-ref guix-pkg
;; "zeal" "qalculate-gtk" "simplescreenrecorder"
"libreoffice" "qview" "gimp"
"nomacs" "tinymediamanager" "simple-scan"
#+end_src
#+begin_src scheme :noweb-ref guix-file
...
** aria2
#+begin_src scheme :noweb-ref guix-pkg
"aria2" "emacs-aria2" "emacs-torrent-mode"
#+end_src
...
#+end_src
#+begin_src cfg :dest ".config/yt-dlp/config"
--format-sort=vcodec:!av01
--remote-components=ejs:github
--js-runtimes quickjs:🐜(pkg "quickjs")🐜/bin/qjs
#+end_src

#+RESULTS[895275df1dce3a89dc24567eff97060594519216]:
#+begin_src text :noweb-ref guix-file
...
(service
 ...
 (make-home-mpv-configuration
  ...
  (make-mpv-profile-configuration
   ...
   #:ytdl-format "bestvideo[height<=?900]+bestaudio/best[height<=?900]/best"
   ...
   (plain-file "input.conf"
               (string-join
                ...
                "\n"))
   #:scripts (list
              (file-append (pkg "mpv-mpris") "/lib/mpv-mpris/mpris.so")
...
#+end_src
#+begin_src scheme :noweb-ref guix-pin
(pin "emacs-telega"
     #:commit "79c173562d3bd14e00e9d05d28bfe6d8dc2a9736"
     #:hash "00bync74r7ywj6c2i9h61rxnxd322zmc416v6ygf7j4psj0xrygq")
#+end_src
#+begin_src elisp
(add-hook 'doom-init-ui-hook (lambda () (telega 'no-popup)))
#+end_src
...
#+begin_src scheme :noweb-ref guix-pin
(pin "emacs-exwm"
     #:commit "369b9126a94c801eb9ec3eddbc00da3236f53bc7"
     #:hash "12v7iiq2yk3pafw4a83innib993cwi33sm4bnh8fawan6a9pgqqz")
#+end_src
#+begin_src elisp
(with-eval-after-load 'exwm
  <<exwm>>
  (add-hook exwm-init #'sarg/autostart-apps))
#+end_src
*** per-app settings
...
(exwm/bind-command
 ...
 "<f13> 1"    (cmd! (tab-bar-switch-to-tab "code"))
 "<f13> 2"    (cmd! (tab-bar-switch-to-tab "brow"))
...
** bar
#+begin_src elisp
(defadvice! +battery-icons (bat-status)
  "Add icons to battery status."
  :filter-return #'battery-upower
  (let ((indicator (assq ?b bat-status)))
    (when (string= (cdr indicator) "+")
      (setf (cdr indicator) "")))
  bat-status)
...
(use-package net-usage
  :custom
  (net-usage-interface "wlp1s0")
  :config
  (net-usage-mode 1))
...
(setopt global-mode-string
        '(emms-mode-line-string
          iwd-manager-mode-line-string
          net-usage-mode-line-string
...
** elfeed
#+begin_src elisp
(doom-load-packages-incrementally '(elfeed))
...
(with-eval-after-load 'browse-url
  (add-to-list 'browse-url-handlers '("bandcamp.com" . browse-url-emms))
  (add-to-list 'browse-url-handlers '("soundcloud.com" . browse-url-emms))
  (add-to-list 'browse-url-handlers '("bandcamp.com/album" . emms-add-ytdlp-playlist)))

(use-package emms
  :defer t
  :commands (emms emms-add-url emms-stop emms-pause emms-previous)
  :init
  ...
  (setopt emms-mode-line-mode-line-function #'emms/modeline-function)
...
#+end_src
*** bindings
#+begin_src scheme :noweb-ref emacs-dep
"wireplumber" "xosd"
#+end_src
#+begin_src elisp
(defalias 'sarg/osd #'ignore)

(let ((my-vol-last-change 0))
  (defun my-vol-change (amount)
    (let* ((time-now (car (time-convert nil 1000)))
           (time-delta (- time-now my-vol-last-change)))

      (thread-last
        (* amount (if (< time-delta 200) 5 2))
        (emms-volume-pulse-change)
        (s-match (rx "Volume is " (group (+ num) "%")))
        (cadr)
        (sarg/osd))

      (setq my-vol-last-change time-now))))

(let ((wpctl (guix/pkg 'wireplumber "/bin/wpctl")))
  (transient-define-prefix dash-menu/audio-mic ()
    "Mic volume menu"
    ["Mic"
     :pad-keys t
     :description (lambda () "Get vol"
                    (let* ((mic-vol (s-chop-prefix "Volume: " (car (process-lines wpctl "get-volume" "@DEFAULT_SOURCE@")))))
                      (format "Mic: %s" mic-vol)))
     ("m" "mute" (lambda () (interactive) (call-process wpctl nil nil nil "set-mute" "@DEFAULT_SOURCE@" "toggle")))
     ("<prior>" "volume up"
      (lambda () (interactive) (call-process wpctl nil nil nil "set-volume" "@DEFAULT_SOURCE@" "5%+"))
      :transient t)
     ("<next>" "volume down"
      (lambda () (interactive) (call-process wpctl nil nil nil "set-volume" "@DEFAULT_SOURCE@" "5%-"))
      :transient t)]))

(transient-define-prefix dash-menu/audio ()
  "Audio menu"
  [["Playlist"
    ("a" "Spotify" consult-spotify-emms)
    ("s" "Systrum" (lambda () (interactive)
                     (emms-play-url "https://systrum.net/SSR2.m3u")))
    ("l" "Likes" (lambda () (interactive)
                   (with-current-emms-playlist
                     (emms-playlist-current-clear)
                     (setopt emms-random-playlist t)
                     (emms-add-directory (concat sarg/media-dir "/music_liked/"))
                     (emms-random))))]
   ["EMMS"
    :pad-keys t
    ("<delete>" "pause" emms-pause)
    ("S-<delete>" "stop" emms-stop)
    ("<right>" "next" (lambda () (interactive)
                        (if emms-random-playlist (emms-random) (emms-next)))
     :transient t)
    ("<left>" "prev" emms-previous :transient t)]

   ["Sound"
    :pad-keys t
    ("m" "mic" dash-menu/audio-mic :transient transient--do-exit)
    ("<prior>" "volume up" (lambda () (interactive) (my-vol-change 1)) :transient t)
    ("<next>" "volume down" (lambda () (interactive) (my-vol-change -1)) :transient t)]])
...
#+begin_src elisp :noweb-ref exwm :tangle no
(let (p)
  (defun sarg/osd (value)
    (when (exwm-layout--fullscreen-p)
      (let ((tmp p))
        (setq p (start-process "osd" nil
                               (guix/pkg 'xosd "/bin/osd_cat")
                               "-P" (if (numberp value) (number-to-string value) value)
                               "-b" "percentage" "-T" "Volume" "-d" "1"
                               "-A" "center" "-c" "blue" "-u" "green" "-o" "50"))
        (when (process-live-p tmp) (delete-process tmp))))))

(exwm/bind-command
 "<XF86AudioRaiseVolume>" (cmd! (my-vol-change 1))
 "<XF86AudioLowerVolume>" (cmd! (my-vol-change -1))
 "<XF86AudioMute>" "pactl set-sink-mute @DEFAULT_SINK@ toggle"
 "<XF86AudioMicMute>" "pactl set-source-mute @DEFAULT_SOURCE@ toggle")

(exwm/bind-command
 "<f13> <deletechar>" #'dash-menu/audio
 "<XF86AudioPlay>" #'emms-play-dwim
 "<XF86AudioPause>" #'emms-stop
 "<XF86AudioPrev>" #'emms-previous
 "<XF86AudioNext>" (cmd! (funcall emms-player-next-function)))
#+end_src
*** spotify
**** player
#+begin_src elisp
...
**** search
#+begin_src elisp
(use-package consult-spotify-emms
  :commands consult-spotify-emms
  :custom
  (consult-spotify-emms-client-id
   (lambda () (auth-source-pass-get "client-id" "Sites/spotify.com")))
  (consult-spotify-emms-client-secret
   (lambda () (auth-source-pass-get "client-secret" "Sites/spotify.com"))))
#+end_src
*** scrobble
Enable scrobbling of URL tracks: https://lists.gnu.org/archive/html/emms-help/2025-12/msg00011.html
#+begin_src elisp
(with-eval-after-load 'emms
  (require 'emms-listenbrainz-scrobbler)

  (defun emms-scrobbler-good-track (track)
    (and (member (emms-track-get track 'type) '(file url))
  ...
  (defun emms-listenbrainz-scrobbler-start-hook ()
    ...
    (when-let* ((current-track (emms-playlist-current-selected-track))
                ((emms-scrobbler-good-track current-track)))
      (emms-listenbrainz-scrobbler-make-async-submission-call current-track t)))
...
*** ytdlp
#+begin_src elisp
(defun emms-add-ytdlp-playlist (url &rest _)
  "Fetch playlist at URL with yt-dlp and add individual tracks to current emms playlist."
  (interactive "sURL: \n")
  (let* ((command (concat "yt-dlp 2>/dev/null -q --flat-playlist --no-download --dump-single-json " url))
    ...
    (seq-do
     ...
     entries)
    (message "Added %d tracks" (length entries))))
#+end_src
** dict
[[https://github.com/BoboTiG/ebook-reader-dict][Wiktionary exports]]
#+begin_src scheme :noweb-ref guix-pkg
"emacs-lexic" "sdcv" "emacs-google-translate" "emacs-dictcc" "emacs-powerthesaurus"
#+end_src
#+begin_src scheme :noweb-ref guix-file
...
#+end_src
#+begin_src scheme :noweb-ref guix-file
(".config/msmtp/config" ,(local-file "../email/.config/msmtp/config"))
#+end_src
*** receiving
#+begin_src scheme :noweb-ref guix-pkg
...
(".config/isyncrc" ,(local-file "../email/.config/isyncrc"))
#+end_src
*** notification
#+begin_src scheme :noweb-ref guix-srv
(online-service
 ...
 (home-goimapnotify-configuration
  (configurations
   (list (goimapnotify-configuration
          (host "imap.gmail.com")
          (port 993)
          (tls? #t)
          (tls-options (goimapnotify-tls-options-configuration))
          (user-name "sarg@sarg.org.ru")
          (password-command "oama access sarg@sarg.org.ru")
          (xo-auth2? #t)
          (boxes
           (list (goimapnotify-box-configuration
                  (mailbox "INBOX")
                  (on-new-mail "emacsclient -e \"(sarg/update-email)\"")
                  (on-new-mail-post "SKIP")
                  (on-changed-mail "emacsclient -e \"(sarg/update-email)\"")
                  (on-changed-mail-post "SKIP")
                  (on-deleted-mail "emacsclient -e \"(sarg/update-email)\"")
                  (on-deleted-mail-post "SKIP")))))))))
#+end_src
*** client
#+begin_src scheme :noweb-ref guix-pkg
"mu" "emacs-consult-mu"
#+end_src
#+begin_src elisp
(defun sarg/update-email ()
  "To be called from goimapnotify."
  (unless (and (featurep 'mu4e) (mu4e-running-p))
    (require 'mu4e))
  (mu4e-update-mail-and-index 'background))

(doom-load-packages-incrementally '(mu4e))
...
* ewm
#+begin_src scheme :noweb-ref guix-pkg
"emacs-ewm" "noctalia-shell" "xwayland-satellite"
#+end_src
#+begin_src scheme :noweb-ref guix-srv
(simple-service
 'emacs
 ...
 (list
  (shepherd-service
   (provision '(emacs))
   (requirement '(dbus))
   (start #~(make-forkexec-constructor (list "emacs" "--fg-daemon" "-f" "ewm-start-module")))
   (stop #~(make-kill-destructor)))))
#+end_src
#+begin_src elisp
(when (daemonp)
...
(defmacro sh! (command)
  "Returns lambda that starts given command in shell."
  (declare (doc-string 1))
  `(lambda (&rest _) (interactive)
     (start-process-shell-command "ewm-bind" nil ,command)))

(defun ewm/switch-to-next-buffer (class &optional reverse?)
...
REVERSE? it when true. Returns the selected buffer."
  (cl-loop
   ...
   when (string= class (buffer-local-value 'ewm-surface-app b)) do
...
(use-package ewm
  :custom
  (ewm-intercept-prefixes '("C-x" "M-x" "<menu>" "C-\\"
                            ("s-f" :fullscreen)
  ...
  :config
  (add-to-list 'doom-real-buffer-modes 'ewm-surface-mode)
  (put 'ewm-surface-mode 'mode-class 'special)
  (when (modulep! :editor evil)
    (add-to-list 'evil-emacs-state-modes 'ewm-surface-mode))
  (map! :map ewm-mode-map
        ...
        "<menu>" (cmd! (unless (minibufferp) (dash-menu)))
        "s-<delete>" #'dash-menu/audio

        "<AudioPlay>" #'emms-play-dwim
        "<AudioPause>" #'emms-stop
        "<AudioPrev>" #'emms-previous
        "<AudioNext>" (cmd! (funcall emms-player-next-function))
        "<AudioRaiseVolume>" (cmd! (my-vol-change 1))
        "<AudioLowerVolume>" (cmd! (my-vol-change -1))
        "<AudioMute>" (sh! "pactl set-sink-mute @DEFAULT_SINK@ toggle")
        "<AudioMicMute>" (sh! "pactl set-source-mute @DEFAULT_SOURCE@ toggle")
        "<MonBrightnessUp>" (sh! "brightnessctl -e set 10%+")
        "<MonBrightnessDown>" (sh! "brightnessctl -e set 10%-")
        "<ScreenSaver>" (sh! "xset s activate")
        "s-<f12>" (sh! "grim -g \"$(slurp -d)\" - | wl-copy -t image/png")

        "s-E"    (cmd!
                  (or (ewm/switch-to-next-buffer "qutebrowser" 'reverse)
                      (start-process "qutebrowser" nil "setsid" "-w" "qutebrowser")))

        "s-e"    (cmd!
                  (or (ewm/switch-to-next-buffer "qutebrowser")
                      (start-process "qutebrowser" nil "setsid" "-w" "qutebrowser"))))

  ;; Floats: no echo area at creation.
  (add-to-list 'display-buffer-alist
               '((major-mode . ewm-surface-mode) nil
                 (pop-up-frame-parameters . ((minibuffer . nil)
                                             (left-fringe . 0) (right-fringe . 0)))))

  (add-hook 'after-make-frame-functions
            (defun +ewm-hide-float-modeline-h (frame)
              (when (frame-parameter frame 'ewm-floating)
...
("QT_PLUGIN_PATH" . ,(file-append (pkg "qtwayland") "/lib/qt6/plugins"))
("QT_QPA_PLATFORM_PLUGIN_PATH" . ,(file-append (pkg "qtwayland") "/lib/qt6/plugins/platforms"))
#+end_src
* COMMENT x11
#+begin_src scheme :noweb-ref guix-pkg
"xset" "xclip"
...
🐜(screen-locker
     "saver_xscreensaver"
     ;(video-saver "/storage/data/backgrounds/snow.mp4")
     #:pam-service (global 'unlock-pam-service))🐜 &
keymap.sh
🐜(pkg "xhost")🐜/bin/xhost +si:localuser:$USER
🐜(pkg "dbus")🐜/bin/dbus-update-activation-environment --verbose DBUS_SESSION_BUS_ADDRESS DISPLAY XAUTHORITY
...
#+RESULTS[4c773088d66fef38b7ff956bf14dae0d59db5bb1]:
#+begin_src text :noweb-ref guix-file
(".xinitrc"
,(mixed-text-file
  "cfg-.xinitrc"
   ...
   (start #~(make-forkexec-constructor
             (list #$(xorg-start-command-xinit
                      (xorg-configuration
                       ...
                       (modules (list xlibre-xf86-input-libinput)))))
             #:create-session? #f))
   (stop #~(make-kill-destructor)))))
#+end_src
* ssh
...
(simple-service
 ...
 (list
  (shepherd-service
    (documentation "noop service indicating online status")
    (provision '(online))
    (auto-start? #f))

  (shepherd-service
    (documentation "updates <online> service state")
    (provision '(online-checker))
                        ...
                        (with-output-to-file *null-device*
                          (lambda _
                            (zero?
                             (cdr
                              (waitpid
                               (spawn "ping" '("ping" "-q" "-c" "1" "-W" "1" "9.9.9.9")))))))))
                  (if is-online
                      (for-each-service
                       (lambda (service)
                         (when (and (member 'online (service-requirement service))
                                    (not (process? (service-running-value service))))
                           (start-service service))))
                      (and=> (lookup-running 'online) stop-service))))))
    (stop #~(compose
             (lambda _ (and=> (lookup-running 'online) stop-service))
...
#+begin_src scheme :noweb-ref guix-srv
(online-service mosquitto-service-type <<cfg-mosquitto()>>)
(online-service owntracks-service-type
         (owntracks-service
...
#+begin_src cfg :dest ".config/user-dirs.dirs"
XDG_DOWNLOAD_DIR="$HOME/Downloads"
XDG_MUSIC_DIR="/storage/media/music"
XDG_VIDEOS_DIR="$HOME/Movies"
#+end_src

#+RESULTS[45e1dae772a995ee3e2b5764b3744a06e9db4ad4]:
#+begin_src text :noweb-ref guix-file
...
** COMMENT smartcard
#+begin_src scheme :noweb-ref system-service
(service (@ (gnu services security-token) pcscd-service-type))
#+end_src
* guix-emacs helper
#+begin_src elisp
(autoload 'guix-read-package-name "guix-package"
  "read package name" t)
(defadvice! +filter-wrong-dirs (directory)
  :before-while #'guix-emacs-find-autoloads
  (file-directory-p directory))

(defun guix-package-use (pkg-name)
  (interactive (list (guix-read-package-name)))

  (guix-emacs--load-file-no-error
   (concat
    (guix-eval-read
     (prin1-to-string
      `(begin (use-modules (guix) (guix profiles) (gnu packages) (ice-9 receive))
        (with-store
         store
         (receive (derivation _)
                  ((lower-object
                    (profile (content (packages->manifest
                                       (list (specification->package ,pkg-name))))))
                   store)
...
(defun guix-package-info-insert-emacs-install-button (id name)
  ...
  (when (string-prefix-p "emacs-" name)
...
(channel
  (name 'doomemacs)
  (url "https://github.com/sarg/guix-doomemacs")
  (branch "master")
  (introduction
   (make-channel-introduction
    "c5c9f26a9cf037979ee58e68ddd83742903d28b1"
    (openpgp-fingerprint
     "09F6 8FC8 0BA0 DC12 5CA7  C490 3ADB 423B 40A2 0785"))))
#+end_src
** base packages
#+begin_src scheme :noweb-ref guix-pkg
home-emacs
...
#+begin_src elisp :tangle packages.el :exports none
; provided by emacs-treemacs-extra
(package! treemacs-evil :built-in t)
(package! treemacs-magit :built-in t)
(package! treemacs-projectile :built-in t)
; don't use and/or don't want to package these
...
(service
 ...
 (doomemacs-configuration
  (emacs home-emacs)
  (doomemacs (pinned (pkg "doomemacs")))
  (config
   (directory-union "config"
                    (list
                     (local-file (relative-file "../emacs/.doom.d") "src"
                                 #:recursive? #t
                                 #:select? (git-predicate (relative-file "../emacs/.doom.d")))

                     (doomemacs-org-config
                      #:src (local-file (relative-file "../emacs/.doom.d/config.org"))
                      #:inputs (map pkg (list <<emacs-dep>>))))))
  (inputs all-packages)))
#+end_src
** guix tweaks
#+begin_src elisp
(defadvice! +set-snippet-dir (fun &rest args)
  "Use writeable snippets dir."
  :around '(+snippets/new +snippets/find-private +snippets/find +snippets/new-alias)
  (let ((+snippets-dir (expand-file-name "~/devel/dotfiles/emacs/.doom.d/snippets")))
    (apply fun args)))
#+end_src
** my lisp code
#+begin_src scheme :noweb-ref guix-pkg
(package
  (name "emacs-sarg-lisp")
  (version "0")
  (source (local-file "../emacs/lisp" #:recursive? #t))
  (build-system (@ (guix build-system emacs) emacs-build-system))
  (arguments '(#:tests? #f))
  (propagated-inputs
   (map pkg '("emacs-promise" "emacs-request" "emacs-s" "emacs-dash")))
  (synopsis "My lisp code")
  (home-page #f)
  (description #f)
  (license #f))
#+end_src
...
#+begin_src scheme :noweb-ref guix-pkg
(package
  (name "emacs-sarg-doom-modules")
  (version "0")
  (source (local-file "../emacs/doommodules" #:recursive? #t))
  (build-system (@ (guix build-system copy) copy-build-system))
  (arguments '(#:install-plan '(("." "share/doomemacs/modules"))))
  (synopsis "My doom modules")
  (home-page #f)
  (description #f)
  (license #f))
#+end_src
** init.el
#+begin_src scheme :noweb-ref guix-pin
;; (pin "doomemacs-modules"
;;      #:commit "b9391c34dbb26df79d804444def3371c1ca8c342"
;;      #:hash "1ia8hs9ma88hkbnrwx1qcynw0bhzqg36mp3c0a29if3rlkrl7vjx")
#+end_src
#+begin_src elisp :tangle init.el
;; generated from config.org
(setq custom-file "~/devel/dotfiles/emacs/.doom.d/custom.el")
(doom!
 :completion  (corfu +icons +orderless) (vertico +icons)
 :ui          nav-flash (modeline +light) ophints (popup +defaults) ligatures treemacs vc-gutter
 ...
 :term        eshell ghostel
 :tools       (eval +overlay) (lookup +docsets) (magit +forge) (pass +auth)
              make tree-sitter lsp llm debugger
 :checkers    syntax
 :lang        (clojure +tree-sitter) data emacs-lisp (javascript +tree-sitter +lsp) (markdown +grip) (scheme +guile)
              (org +dragndrop +pandoc +roam2 +pretty) beancount (go +lsp +tree-sitter) (rust +lsp +tree-sitter)
              (rest +jq) (sh +tree-sitter) plantuml (python +lsp +tree-sitter) (web +tree-sitter) yaml (json +tree-sitter)
 :app         telega (rss +org) torrent irc dash-menu
 :email       (mu4e +mbsync +gmail)
...
(package
  (name "guix-artwork")
  (version "0")
  (source (@ (gnu artwork) %artwork-repository))
  (build-system copy-build-system)
  (license #f) (home-page #f) (synopsis #f) (description #f))
#+end_src
#+begin_src elisp
(setq fancy-splash-image (guix/pkg 'guix-artwork "/logo/head-only/Guix-head-stencil.svg"))
(setq +doom-dashboard-functions
      '(doom-dashboard-widget-shortmenu))
#+end_src
** modeline
#+begin_src elisp
(setopt +modeline-encoding nil
        +modeline-height 20)
...
"emacs-greymatters-theme"
"emacs-doom-themes"
#+end_src
#+begin_src scheme :noweb-ref guix-pin
(pin "emacs-doom-themes"
     #:commit "53645a905dfb3055db52f5d418d5ef612027e062"
     #:hash "1pqmk6mi1nk3axkb0zfcy7x8vqz5984km0vqp6mmh0sd2ysc4rdl")
#+end_src
#+begin_src elisp
(use-package doom-themes
  :defer t
  :config
  (setcdr (assoc 'tab-line doom-themes-base-faces)
          '(:background bg-alt :foreground fg-alt))
  
  (setcdr (assoc 'gnus-group-news-low-empty doom-themes-base-faces)
          '(:inherit 'gnus-group-mail-1-empty :weight 'normal)))
(defvar doom-theme-dark 'doom-tomorrow-night)
...
#+begin_src elisp
(use-package ben
  :hook (doom-first-file . ben-global-mode)
  :config
  ; FIXME: upstream bug that fails when default-directory is set to nil
  (defun ben--default-directory-watcher (_ newval _ buf)
    (unless (or (null newval) (file-remote-p newval))
...
(add-hook 'doom-init-ui-hook #'sarg/autostart-apps)
#+end_src
* flatpaks
#+begin_src scheme :noweb-ref guix-env-var
...
#+begin_src scheme :noweb-ref guix-file
(".icons/default"
    ,(file-append (pkg "hackneyed-x11-cursors") "/share/icons/Hackneyed"))
#+end_src
#+begin_src scheme :noweb-ref guix-env-var
...
#+RESULTS[65b7c393c6a4b27468c44ac7b2a34cc0a3e4c042]:
#+begin_src text :noweb-ref guix-file
(".config/associations"
,(mixed-text-file
  "cfg-file"
...
#+begin_src cfg :dest ".local/share/darkman/noctalia.sh" :chmod "u+x"
#!/bin/sh
export WAYLAND_DISPLAY=$(
    cd $XDG_RUNTIME_DIR
    ls -1 wayland-ewm-* | grep -v lock
)
noctalia msg theme-mode-set $([ "$1" = "light" ] && echo light || echo dark)
noctalia msg wallpaper-set ~/Pictures/Wallpapers/$1.jpg
#+end_src

#+RESULTS[f37ddc021d4e87ba0b0b8b132afe5555d65bdbb1]:
#+begin_src text :noweb-ref guix-file
(".local/share/darkman/noctalia.sh"
...
")\n"
"noctalia msg theme-mode-set $([ \"$1\" = \"light\" ] && echo light || echo dark)\n"
"noctalia msg wallpaper-set ~/Pictures/Wallpapers/$1.jpg") #o744))
#+end_src

#+begin_src elisp
(use-package darkman
  :hook (doom-init-ui . darkman-mode)
...
** kdeconnect
#+begin_src scheme :noweb-ref guix-pkg
"kdeconnect" "emacs-kdeconnect"
#+end_src
#+begin_src elisp
(use-package kdeconnect
  :hook (doom-init-ui . kdeconnect-refresh)
  :defer t
  :custom
  (kdeconnect-active-device '("Pixel 9a" . "96c0a00f623640859f01c89f53db3b45")))
#+end_src
* miscellaneous
#+begin_src elisp
...
(package! toc-org :disable t) ; hangs blog.org
#+end_src
#+begin_src elisp
(setq
...
*** template
#+begin_src elisp
(with-eval-after-load 'org-project-capture
...
(defun sarg/magit-clone-clean-url (url)
  (cond
   ((string-match "^.+github.com/[^/]+/[^/]+" url)
    (substring url 0 (match-end 0)))
   (t url)))

(defun sarg/magit-clone-or-open (url)
  (let* ((clean-url (sarg/magit-clone-clean-url url))
         (target-dir
          (expand-file-name (magit-clone--url-to-name clean-url)
                            magit-clone-default-directory)))
    (if (magit-git-repo-p target-dir)
        (magit-status target-dir)
      (magit-clone-regular clean-url magit-clone-default-directory nil))
    t))
...
** just
#+begin_src scheme :noweb-ref guix-pkg
"just" "emacs-just-mode" "emacs-justl"
#+end_src
...
#+end_src
#+begin_src cfg :dest ".config/git/config"
[user]
    name = 🐜(global 'name)🐜
    email = 🐜(global 'email)🐜
...
#+RESULTS[4abbf0219506add0d6dd52908c37ac75836e1b24]:
#+begin_src text :noweb-ref guix-file
(".config/git/config"
,(mixed-text-file
  "cfg-config"
...
#+begin_src cfg :dest ".config/git/ignore"
.direnv
.vscode
__pycache__/
*.py[cod]
...
#+RESULTS[caf5025bcf1606e1512b769536d3a41c2102c1c5]:
#+begin_src text :noweb-ref guix-file
(".config/git/ignore"
,(mixed-text-file
  "cfg-file"
...
(with-eval-after-load 'magit
  ...
  (transient-define-suffix magit-push-current-agit (source target args)
    :if (lambda () (magit-get-boolean "remote" (magit-get-remote) "agit"))
    ...
    (let* ((origin (magit-get "branch" source "remote"))
           ...
           (parts (and desc (s-split-up-to "\n\n" desc 1)))
           (title (and parts (car parts)))
           (body (and parts (cadr parts))))
      ...
      (magit-push-refspecs
       ...
       `(,@args
         "-o" "force-push=true"
         ,@(if title (list "-o" (concat "title=" title)) '())
         ,@(if body (list "-o" (concat "description={base64}" (base64-encode-string body :no-line-break)) '()))))))

  (defsubst magit--branch-description-template ()
    (magit-get "remote" (magit-get-remote) "descriptionTemplate"))

  (transient-define-suffix magit-branch-description-template (branch)
    :if (lambda () (and (not (magit-get "branch" (magit-get-current-branch) "description"))
                        (magit--branch-description-template)))
    :description (lambda () (concat "set branch description from " (magit--branch-description-template)))
    (interactive (list (oref transient-current-prefix scope)))

    (magit-call-git "config" (concat "branch." branch ".description")
                    (f-read (magit--branch-description-template)))
    (magit-branch.<branch>.description branch))

  (transient-insert-suffix 'magit-branch
    '(0 0) (list "t" #'magit-branch-description-template))
...
** guix development
#+begin_src scheme :noweb-ref guix-pkg
"emacs-debbugs" "gnu-standards" "emacs-fj" "emacs-guix"
(info-package (pkg "glibc"))
#+end_src
#+begin_src elisp
...
(use-package fj
  ...
  :config
  (setopt fj-host "https://codeberg.org"
          fj-user "sarg"
...
** rust
#+begin_src scheme :noweb-ref guix-pkg
"emacs-rust-mode" "emacs-rustic" "tree-sitter-rust"
#+end_src
#+begin_src scheme :noweb-ref emacs-dep
"lldb" "codelldb"
#+end_src
...
(with-eval-after-load 'dape
  ; lsp-mode runs cargo with incomplete PATH otherwise
  (advice-add 'shell-command-to-string :around #'ben-propagate-environment)

  ; lsp-mode with dape support
  (defun dap-debug (cfg)
    (dape (dape--config-eval 'rust `(:program ,(plist-get cfg :program)))))

  (let ((tmp (make-temp-file "config")))
    (with-temp-file tmp
      (insert "(provide 'dap-gdb)"))
    (require 'dap-gdb tmp))

  (add-to-list 'dape-configs
               '(rust modes (rust-mode rust-ts-mode) command-args
                 ("--port" :autoport "--settings" "{\"sourceLanguages\":[\"rust\"]}"
                  "--liblldb" (guix/pkg 'lldb "/lib/liblldb.so"))
                 ensure dape-ensure-command command-cwd dape-command-cwd command
                 (guix/pkg 'codelldb "/extension/adapter/codelldb")
                 port :autoport :type "lldb" :request "launch" :cwd "." :args []
                 :stopOnEntry nil)))
#+end_src
** clojure
install grammars as expected by clojure-ts-mode
#+begin_src scheme :noweb-ref guix-pkg
((@@ (gnu packages tree-sitter) tree-sitter-grammar)
 "regex" "regex"
 "098pkjicnw3a8bypb7f2xqwhqjlfvicibcmgx5nsb0icdyvagm8q"
 "0.24.3"
 #:repository-url "https://github.com/tree-sitter/tree-sitter-regex")

((@@ (gnu packages tree-sitter) tree-sitter-grammar)
 "clojure" "Clojure"
 "1jhwsif4rm8dc853ad3i9zrgnrps9krpb65lhgcwl8r4zxlspj7q"
 "unstable-20250526"
 #:commit "69070d2e4563f8f58c7f57b0c8e093a08d7a5814"
 #:repository-url "https://github.com/sogaiu/tree-sitter-clojure")

(package
  (inherit (pkg "tree-sitter-markdown"))
  (name "tree-sitter-markdown_inline")
  (arguments
   (list #:phases
         #~(modify-phases %standard-phases
             (add-after 'install 'copy-grammar
               (lambda _
                 (symlink (string-append #$output "/lib/tree-sitter/libtree-sitter-markdown_inline.so")
                          (string-append #$output "/lib/tree-sitter/libtree-sitter-markdown-inline.so")))))
         #:grammar-directories '("tree-sitter-markdown" "tree-sitter-markdown-inline"))))
#+end_src
** elisp
#+begin_src scheme :noweb-ref guix-pkg
"emacs-visual-shorthands"
#+end_src
#+begin_src elisp
(use-package visual-shorthands
  :commands visual-shorthands-mode

  :init
  (face-spec-set 'elisp-shorthand-font-lock-face
                 '((t :inherit nil :foreground nil))))
#+end_src
* tramp
#+begin_src scheme :noweb-ref guix-pkg
"emacs-tramp-hlo"
...
** qutebrowser
#+begin_src scheme :noweb-ref guix-pin
(pin "emacs-qutebrowser"
     #:uri "https://github.com/sarg/qutebrowser.el"
     #:commit "23d507b93a458b95c615a8442dad4991fdc03407"
     #:hash "0n2a3ad2c3zry4hci1242w0nj7h082hhsdfngdxzy537viqjhmah")
#+end_src
#+begin_src scheme :noweb-ref guix-pkg
"qutebrowser" "emacs-qutebrowser"
#+END_src
#+begin_src scheme :noweb-reg guix-env-var
("BROWSER" . "qutebrowser")
#+end_src
#+begin_src cfg :dest .config/qutebrowser/blank.html
<!DOCTYPE html>
<html><body style="min-height: 90vh; display: grid; place-items: center;">
   <img src="file://🐜(@ (gnu artwork) %artwork-repository)🐜/logo/head-only/Guix-head-stencil.svg">
</body></html>
#+end_src

#+RESULTS[c86319cfa1b269186e09c2b2d86921888fcf1196]:
#+begin_src text :noweb-ref guix-file
(".config/qutebrowser/blank.html"
,(mixed-text-file
  "cfg-blank.html"
"<!DOCTYPE html>\n"
"<html><body style=\"min-height: 90vh; display: grid; place-items: center;\">\n"
...
(defun qutebrowser-act-dwim (url)
  (interactive)
  (funcall (or (browse-url-select-handler url)
               #'sarg/magit-clone-or-open)
           url))

(setq-default browse-url-browser-function #'qutebrowser-open-url)

(when (and (modulep! :app exwm) (modulep! :editor evil))
...
(use-package qutebrowser
  :autoload qutebrowser-rpc-connect
  :hook ((darkman-mode . qutebrowser-theme-export-mode)
         (exwm-init . global-qutebrowser-exwm-mode)
         (exwm-init . qutebrowser-evil-state-mode)
         (exwm-init . qutebrowser-theme-export-mode))
  :custom
  (qutebrowser-pass-username-function (fn! (auth-source-pass-get "login" %)))
  (qutebrowser-rpc-timeout 1.5)
  ...
  (add-hook! 'qutebrowser-theme-export-functions
    (insert "c.colors.webpage.darkmode.enabled = "
            (if (string= (darkman-current-mode) "dark") "True" "False")
            "\n"))
...
(use-package yeetube
  ...
  :custom
  (yeetube-play-function #'browse-url-mpv)
  :config
  (map! :map yeetube-mode-map
        :n "RET" #'yeetube-play
        :n "go" (cmd! (browse-url-generic (yeetube-get-url)))))
#+end_src
** hnreader
#+begin_src scheme :noweb-ref guix-pkg
"emacs-hnreader"
...
(use-package hnreader
  ...
  :config
  (with-eval-after-load 'browse-url
    (add-to-list 'browse-url-handlers (cons (rx "news.ycombinator.com/item?id=")
                                            (lambda (url &rest _) (hnreader-comment url)))))
  (org-link-set-parameters "eww" :follow 'eww-browse-url))
#+end_src
* shell
#+begin_src scheme :noweb-ref guix-pkg
"emacs-eat" "emacs-detached" "direnv" "atuin" "emacs-eshell-atuin" "mxp"
#+end_src
...
(use-package detached
  :disabled ; causes freeze-ups in eshell
  :defer-incrementally t
  :config
  (detached-init)
...
* dired
#+begin_src scheme :noweb-ref guix-pkg
"emacs-dired-hacks" "emacs-dired-du" "emacs-sxiv"
#+end_src

#+begin_src elisp
(defun dired-do-materialize-symlink ()
  "Replace selected symlinks with copies of their target files."
  (interactive)
  (dolist (file (dired-get-marked-files))
    (let ((target (file-symlink-p file)))
      (cond
       ...
       ((yes-or-no-p (format "Replace %s with copy of %s? " file target))
        (delete-file file)
        (copy-file target file t)
...
(use-package dired-du
  :after dired
  :custom (dired-du-size-format t)) ;; human readable
...
(with-eval-after-load 'dired-x
  (setq dired-guess-shell-alist-user
        '(("\\.\\(?:mp4\\|mkv\\|webm\\|avi\\)\\(?:\\.part\\)?\\'" "jaro"))))

(use-package dired-open
  :after dired
  :custom
  (dired-open-extensions-elisp `(("torrent" . aria2-download)))
...
(with-eval-after-load 'gptel
  (let ((openrouter
         (gptel-make-openai "OpenRouter"
           ...
           :stream t
           :key (lambda (&rest args) (gptel-api-key-from-auth-source "Api" "openrouter.ai"))
...
(use-package agent-shell
  ...
  (setq agent-shell-preferred-agent-config '(preselect . pi)
        agent-shell-pi-acp-command (list (guix/pkg 'pi-acp "/bin/pi-acp"))
        agent-shell-pi-environment
        (agent-shell-make-environment-variables
         "PI_ACP_PI_COMMAND" (guix/pkg 'pi-coding-agent "/bin/pi"))))
#+end_src
...
(use-package cyclekey
  ...
  :custom (cyclekey-save-languages nil)
...
(use-package gpx
  :after osm
  :config
  (defun gpx-show-map-gpxsee (file track segment)
          (split-window-right)
          (windmove-right)
          (async-start-process "gpxsee" "gpxsee" nil file))
  (setq gpx-show-map-function
        (defun gpx-show-map-osm (file track segment) (osm-open file))))
#+end_src
** emacs-dmenu
#+begin_src elisp
(cl-defun dmenu/select-line-from-file (file &key prompt lines require-match)
  "completing-read a line from FILE."
  (condition-case err
      (completing-read (concat (or prompt "") " ")
                       (with-temp-buffer
    ...
    (quit nil)))
#+end_src
#+begin_src cfg :dest ".local/bin/dmenu" :chmod "u+x"
#!/bin/sh
prompt=nil
lines=nil
require=nil
while getopts ":p:l:r" opt; do
    ...
    r)
        ...
        ;;
    ,*)
        echo "Usage: $0 -p prompt -l lines -r"
        exit 1
        ;;
    esac
done
...
tmp=$(mktemp)
cat >$tmp
result=$(emacsclient -e '(dmenu/select-line-from-file "'$tmp'" :prompt '"$prompt"' :lines '$lines' :require-match '$require')')
if [ "nil" = "$result" ]; then
    exit 1
else
    echo "$result" | jq -r .
...
#+RESULTS[44a1610aa48104365212d18b75f0aefdfb097b73]:
#+begin_src text :noweb-ref guix-file
(".local/bin/dmenu"
,(chmod-computed-file (mixed-text-file
  "cfg-file"
