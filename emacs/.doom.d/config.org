...
(defvar sarg/media-dir "/media/500GB/media")
#+end_src
* system.scm
** template
...
#+begin_src elisp
(defun sarg/aria2-notify-completion ()
  ...
  (let* (timer
         (fn (lambda ()
               (when (or (not aria2--cc)
                 ...
                 (cancel-timer timer)))))
    (setq timer (run-with-timer 0 10 fn))))
#+end_src
...
(package! qutebrowser :recipe
  (:host github :repo "sarg/qutebrowser.el" :files (:defaults "*.py")))
#+end_src
#+begin_src scheme :noweb-ref guix-pkg
"qutebrowser"
#+END_src
