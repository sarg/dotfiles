...
(simple-service
 ...
 (list
  (shepherd-service
   ...
   (requirement '(x11-display gpg-agent online))
...
(use-package ewm
  :custom
  (ewm-input-config '((keyboard :xkb-layouts "us,ru")))
  (ewm-intercept-prefixes '("C-x" "M-x" "<menu>"))
  :config
  (map! :map ewm-mode-map
        "s-r"     #'xdg-launcher-run-app
        ...
        "S-s-<f12>" (sh! "flameshot gui")
        "s-<f12>" (sh! "flameshot gui -c -s")
...
(service screen-locker-service-type
         (screen-locker-configuration
           (name "swaylock")
           (program (file-append (pkg "swaylock-fprintd") "/bin/swaylock"))
           (using-pam? #t)
           (using-setuid? #f)))
#+end_src
* x11
...
(pin "emacs-evil-collection"
     ...
     #:hash "1q9w1wwlfcvw9xhpcv56qx9y6g2wrfrl1mcs2gzbds9qb2nszf7y")
(pin "emacs-evil-org"
...
(doom!
 ...
 :term        eshell vterm
 :tools       editorconfig (eval +overlay) (lookup +docsets) (magit +forge) (pass +auth)
...
"flameshot" "grim" ; needs to be in the profile for dbus facility
"brightnessctl"
#+end_src
#+begin_src elisp :noweb-ref exwm :tangle no
(exwm/bind-command
...
(use-package darkman
  ...
  (if (display-graphic-p)
    (add-hook! 'after-init-hook :depth -95 #'darkman-mode)))
#+end_src
** redshift
#+begin_src scheme :noweb-ref guix-srv
(service
 ...
 (list
  (shepherd-service
   ...
   (modules `(((shepherd support) #:hide (mkdir-p)) ;for '%user-log-dir'
              ...
              ,@(@ (gnu services shepherd) %default-modules)))
   ;; not make-forkexec-constructor as it doesn't observe DISPLAY changes
   (start #~(lambda _
              (fork+exec-command
               (list #$(file-append (pkg "kdeconnect") "/bin/kdeconnectd"))
               #:environment-variables
               (cons (string-append "DISPLAY=" (getenv "DISPLAY"))
                     (remove (cut string-prefix? "DISPLAY=" <>)
                             (default-environment-variables))))
              #:log-file (string-append %user-log-dir "/kdeconnectd.log")))
   (stop #~(make-kill-destructor)))))
#+end_src
* miscellaneous
#+begin_src elisp
...
(defun sarg/magit-clone-or-open (url)
  (let ((target-dir
    ...
    (if (magit-git-repo-p target-dir)
      ...
      (magit-clone-regular (sarg/magit-clone-clean-url url)
                           magit-clone-default-directory nil))
...
* tramp
#+begin_src scheme :noweb-ref guix-pkg
; build with emacs-next to avoid version mismatch warning
(let* ((emacs-tramp (pkg "emacs-tramp"))
       (emacs-tramp-next (build-with-home-emacs emacs-tramp))
       (r (package-input-rewriting `((,emacs-tramp . ,emacs-tramp-next)))))
  (list emacs-tramp-next
        (r (pkg "emacs-tramp-hlo"))))
#+end_src
#+begin_src elisp
