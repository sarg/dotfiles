  ...
  (make-mpv-profile-configuration
   ...
   (plain-file "input.conf"
               (string-join
                ...
                "\n"))
   #:scripts (list
              (file-append (pkg "mpv-mpris") "/mpv-mpris/lib/mpris.so")
...
(add-hook 'doom-init-ui-hook
          (defun sarg/noctalia ()
            ...
            (make-process :buffer " *noctalia*"
                          :name "noctalia"
                          :command '("noctalia-shell" "--no-color")
                          :noquery t)))
...
(use-package ewm
  ...
  (map! :map ewm-mode-map
        "s-h"     #'ewm-windmove-left
        "s-j"     #'evil-window-down
        "s-k"     #'evil-window-up
        "s-l"     #'ewm-windmove-right

        "s-1" (cmd! (tab-bar-switch-to-tab "code"))
        "s-2" (cmd! (tab-bar-switch-to-tab "brow"))
        ...
        "s-E"    (cmd!
                  ...
                  (cond
                   ((string= "qutebrowser" exwm-class-name)
                    (ewm/switch-to-next-buffer "org.qutebrowser.qutebrowser" 'reverse))
        ...
        "s-e"    (cmd!
                  (tab-switch "brow")
                  (or (ewm/switch-to-next-buffer "org.qutebrowser.qutebrowser")
                      (start-process "qutebrowser" nil "setsid" "-w" "qutebrowser"))))
...
noctalia-shell ipc call darkMode set$([ "$1" = "light" ] && echo Light || echo Dark)
noctalia-shell ipc call wallpaper set ~/Pictures/Wallpapers/$1.jpg eDP-1
#+end_src

#+RESULTS[305bcc99468f8c46568ac6a43e4161d73672af2f]:
#+begin_src text :noweb-ref guix-file
...
")\n"
"noctalia-shell ipc call darkMode set$([ \"$1\" = \"light\" ] && echo Light || echo Dark)\n"
"noctalia-shell ipc call wallpaper set ~/Pictures/Wallpapers/$1.jpg eDP-1") #o744))
#+end_src
...
(use-package gpx
  ...
  (setq gpx-show-map-function
        (defun gpx-show-map-osm (file track segment) (osm-gpx-show file))))
#+end_src
** emacs-dmenu
#+begin_src elisp
