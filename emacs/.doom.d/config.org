...
(use-package! desktop-environment
  ...
  (after! emms
    ...
    (advice-add 'desktop-environment-music-next :override #'emms-next)
...
(package! emms-player-spotify
  ...
  :pin "5181c1d14b37ca71e01375c89de200fe50e6cadc")
...
(use-package! counsel-spotify
  ...
  (cl-defmethod counsel-spotify-format :around (element)
    (propertize (cl-call-next-method) 'spotify-object element))
    ...
    (counsel-spotify-with-auth-token (auth-token)
     (counsel-spotify-with-query-results
      ...
      (with-current-emms-playlist
        (goto-char (point-max))
        (mapc (counsel-spotify-parse-items (list (cons 'tracks results)) 'tracks)
          (lambda (el)
      ...
      (cond
       ...
       (t
        (let ((track (emms-track 'url (uri playable))))
          ...
          (emms-track-set track 'info-title (name playable))
          (emms-playlist-insert-track track)
